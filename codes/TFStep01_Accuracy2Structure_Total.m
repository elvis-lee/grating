...
for F = frequency_range
    ...
    %% load result_01
    load([file_location '/Total_Accuracy_freq=' num2str(F)], 'Accuracy');
...
%% show figures
figure(1);imagesc(TimeFreq_Map1);set(gca,'YDir','normal');set(gca,'ytick',1:length(frequency_range),'yticklabel',frequency_range);colorbar;colormap jet;caxis(disp_prob_range);title('Total Response-All Groups');
...
figure(2);imagesc(TimeFreq_Map2);set(gca,'YDir','normal');set(gca,'ytick',1:length(frequency_range),'yticklabel',frequency_range);colorbar;colormap jet;caxis(disp_prob_range);title('Total Response-Group 1(B/W)');
hold on; plot([onset_time onset_time],[0 length(frequency_range)+1],'r','Linewidth',3);
hold on; plot([end_time end_time],[0 length(frequency_range)+1],'r','Linewidth',3);
figure(3);imagesc(TimeFreq_Map3);set(gca,'YDir','normal');set(gca,'ytick',1:length(frequency_range),'yticklabel',frequency_range);colorbar;colormap jet;caxis(disp_prob_range);title('Total Response-Group 2(R/G)');
hold on; plot([onset_time onset_time],[0 length(frequency_range)+1],'r','Linewidth',3);
hold on; plot([end_time end_time],[0 length(frequency_range)+1],'r','Linewidth',3);
figure(4);imagesc(TimeFreq_Map4);set(gca,'YDir','normal');set(gca,'ytick',1:length(frequency_range),'yticklabel',frequency_range);colorbar;colormap jet;caxis(disp_prob_range);title('Total Response-Group 3(Swapped)-All');
hold on; plot([onset_time onset_time],[0 length(frequency_range)+1],'r','Linewidth',3);
hold on; plot([end_time end_time],[0 length(frequency_range)+1],'r','Linewidth',3);
figure(5);imagesc(TimeFreq_Map5);set(gca,'YDir','normal');set(gca,'ytick',1:length(frequency_range),'yticklabel',frequency_range);colorbar;colormap jet;caxis(disp_prob_range);title('Total Response-Group 3(Swapped)-Strong');
hold on; plot([onset_time onset_time],[0 length(frequency_range)+1],'r','Linewidth',3);
hold on; plot([end_time end_time],[0 length(frequency_range)+1],'r','Linewidth',3);

%% save figures
saveas(1,[save_fig_location 'total_response_all_groups.bmp']);
saveas(2,[save_fig_location 'total_response_group_1_bw.bmp']);
saveas(3,[save_fig_location 'total_response_group_2_gr.bmp']);
saveas(4,[save_fig_location 'total_response_group_3_all.bmp']);
saveas(5,[save_fig_location 'total_response_group_3_strong.bmp']);

%% save mat data
save([save_map_location 'total_time_freq_decoding_map'],'TimeFreq_Map1','TimeFreq_Map2','TimeFreq_Map3','TimeFreq_Map4','TimeFreq_Map5');